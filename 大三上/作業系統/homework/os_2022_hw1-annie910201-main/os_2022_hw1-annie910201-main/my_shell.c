  ...
  if(strcmp(args[1],"-i")==0){      //if find this process pid
    ...
    if(fp == NULL){
      ...
      while(getline(&line_of_file,&t, fp)!=EOF){
        ...
        char *loc1 = strstr(line_of_file,"PPid");
        if(loc1 != NULL){
          int k=0;
...
int pipeline(char** args,int command_num){
  ...
  for(int i=0;i<command_num;i++){
    ...
    if(pid == 0){ //child
      ...
      if(i == command_num - 1 && back_ground_pattern)
        printf("[Pid]: %d\n", back_pid);
...
int lsh_execute(char **args)
{
  ...
  if(back_ground_pattern == 1 && pipeline_pattern != 1){//if only have background
    back_ground(args);
    printf("[Pid]: %d\n",getpid());
    int status;
    pid_t pid;
...
char *lsh_read_line(void)
{
  ...
  while (1) {//read input, and check if space is out of limit
...
char **lsh_split_line(char *line)
{
  ...
  while (token != NULL) {
    ...
    if (position >= bufsize) {//out of limit, neew to realloc
      bufsize += LSH_TOK_BUFSIZE;
      tokens_backup = tokens;
      tokens = realloc(tokens, bufsize * sizeof(char*));
      if (!tokens) {//realloc error, like there is nothing in tokens
		    free(tokens_backup);
        fprintf(stderr, "lsh: allocation error\n");
        exit(EXIT_FAILURE);
      }
  ...
  do {
    ...
    if(!strstr(line,"replay")){//no replay, just record current command
      if(tail <= 15){//if doesn't over the space limit, just malloc a new space and put it in the history
        history[tail] = (char*)malloc(sizeof(char) * strlen(line));//create a "line" spacs for every history row 
      ...
      if(number >= 1 && number < tail+1){//check if out of limit
    ...
    /* if there are pipeline */ 
    if(pipeline_pattern){
      int command_num = count_command(args);
      status = pipeline(args,command_num);
    } else 
